<!DOCTYPE html>
<html>
<head>
</head>
<body>
<a href="../">Volver</a>

<table align="center">
<h3 align="center">INGRESO AL SISTEMA | Alumnos <img src="../admin/img/p1.gif"> <img align="center" src="img/key.gif" height="40px" width="40px">
</h3> 
  <tr>
<form method="post" action="validar_ingreso.php">
    <td> Docuemento: </td><td> <input type="text" name="usuario" required="required"> </td>
  </tr>
  <tr>
    <td> Clave: </td> <td><input type="password" name="clave" required="required">  </td>
  </tr>
  <tr>
    <td> <input type="submit" value="Ingresar"> </td><td> <a href="#">Si olvido su clave, haga clic aqui.</a></td>
  </tr>
</table>
</form>
<br>
<hr>
<br>
<i>2013 - Informatica</i>

</body>
</html>
